...
const Cards = () => {
  return (
    <>
      ...
      <div className='bg-gray-100 inline-block rounded-lg mt-5 w-fit mr-28 ml-28 p-2'>
        <h2 className='mt-3 text-2xl text-left non-italic ml-2'>Promotional Offers</h2>
        {/* First Set of Cards (1-3) */}
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 mt-2 mx-2 gap-5'>
          {[ 
            ...
            <div key={index} className='card bg-base-100 image-full w-full h-55 shadow-xl hover:scale-105 transform transition duration-300 ease-in-out'>
              <figure>
                <img src={src} className='w-full h-55 object-cover rounded-lg' alt='Flight Destination' />
              </figure>
            </div>
          ))}
        </div>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 mt-4 mx-2 gap-5'>
          {[ 
